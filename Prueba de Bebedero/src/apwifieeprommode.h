...
/*        if (posW == 0)
        ...
        // Mostrar Ip de conexión
        IPAddress ip = WiFi.localIP();
        
        String html = "<html><body>";
        html += "<p>¡Conexión establecida!</p>";
        html += "<p><strong>La IP de tu ESP32 es:</strong> ";
        html += ip.toString();
        html += "</p>";
        html += "<p>En la app, usa esta IP para comunicarte vía WebSocket</p>";
        html += "</body></html>";
        server.send(200, "text/html", html);
...
void intentoconexion(const char *apname, const char *appassword)
{
    ...
    {                               // redirige a la funcion
        ...
        Serial.println("Conectarse desde su celular a la red creada: SSID:Bebedero-Config, Pass:Amadeus2025");
